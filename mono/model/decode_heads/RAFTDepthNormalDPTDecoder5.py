...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], out_dims=2, tuning_mode=None):
        ...
        self.n_downsample = (
            args.model.decode_head.n_downsample
        )  # 3, resolution of the disparity field (1/2^K)
        ...
        self.flow_head = FlowHead(
            hidden_dims[2], hidden_dim=2 * hidden_dims[2], tuning_mode=tuning_mode
        )
    ...
    def __init__(self, in_dim, output_dim, tuning_mode=None):
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(
                    in_dim[0], dim[0], "layer", stride=1, tuning_mode=tuning_mode
                ),
                Conv2dLoRA(
                    dim[0], dim[0], 3, padding=1, r=8 if tuning_mode == "lora" else 0
                ),
            )
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(
                    in_dim[1], dim[1], "layer", stride=1, tuning_mode=tuning_mode
                ),
                Conv2dLoRA(
                    dim[1], dim[1], 3, padding=1, r=8 if tuning_mode == "lora" else 0
                ),
            )
            output_list.append(conv_out)
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(
                    in_dim[2], dim[2], "layer", stride=1, tuning_mode=tuning_mode
                ),
                Conv2dLoRA(
                    dim[2], dim[2], 3, padding=1, r=8 if tuning_mode == "lora" else 0
                ),
            )
            output_list.append(conv_out)
...
class Readout(nn.Module):
    # From DPT
    def __init__(
        self, in_features, use_cls_token=True, num_register_tokens=0, tuning_mode=None
    ):
        super(Readout, self).__init__()
        self.use_cls_token = use_cls_token
        if self.use_cls_token == True:
            self.project_patch = LoRALinear(
                in_features, in_features, r=8 if tuning_mode == "lora" else 0
            )
...
class Token2Feature(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(self.scale_factor, float):
            x = interpolate_float32(
                x.float(), scale_factor=self.scale_factor, mode="nearest"
            )
        x = self.sample(x)
        return x
...
class DecoderFeature(nn.Module):
    def __init__(
        ...
        self.upconv_2 = FuseBlock(
            self.num_ch_dec[3], self.num_ch_dec[2], tuning_mode=tuning_mode
        )
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.feature_channels = (
            cfg.model.decode_head.feature_channels
        )  # [256, 512, 1024, 1024] [2/7, 1/7, 1/14, 1/14]
        ...
        self.hidden_dims = self.context_dims = (
            cfg.model.decode_head.hidden_channels
        )  # [128, 128, 128, 128]
        self.n_gru_layers = cfg.model.decode_head.n_gru_layers  # 3
        self.n_downsample = (
            cfg.model.decode_head.n_downsample
        )  # 3, resolution of the disparity field (1/2^K)
        ...
        self.decoder_mono = DecoderFeature(
            self.in_channels, self.decoder_channels, tuning_mode=tuning_mode
        )
    ...
    def get_bins(self, bins_num):
        depth_bins_vec = torch.linspace(
            math.log(self.min_val), math.log(self.max_val), bins_num, device="cuda"
        )
    ...
    def regress_depth(self, feature_map_d):
        ...
        d = compute_depth_expectation(
            prob, self.depth_expectation_anchor[:B, ...]
        ).unsqueeze(1)
    ...
    def upsample(self, x, scale_factor=2):
        ...
        return interpolate_float32(
            x, scale_factor=scale_factor * self.up_scale / 8, mode="nearest"
        )
    ...
    def forward(self, vit_features, **kwargs):
        ...
        if self.use_cls_token == True:
            vit_features = [
                [
                    ft[:, 1 + num_register_tokens :, :].view(
                        B, H, W, self.in_channels[0]
                    ),
            ...
            vit_features = [
                ft.view(B, H, W, self.in_channels[0]) for ft in vit_features
            ]
        ...
        feature_map = ref_feat[
            :, :-2, :, :
        ]  # feature map share of depth and normal prediction
        ...
        normal_pred = self.pred_normal(
            feature_map, normal_confidence_map
        )  # mlp for normal

        depth_init = torch.cat(
            (depth_pred, depth_confidence_map, normal_pred), dim=1
        )  # (N, 1+1+4, H, W)
        ...
        net_list = [
            torch.tanh(x[0]) for x in cnet_list
        ]  # x_4, x_8, x_16 of hidden state
        inp_list = [
            torch.relu(x[1]) for x in cnet_list
        ]  # x_4, x_8, x_16 context features
        ...
        if self.training:
            ...
            flow_predictions = [
                self.clamp(init_depth[:, :1] * self.regress_scale + self.max_val)
            ]
        ...
        for itr in range(self.iters):
            ...
            flow_predictions.append(
                self.clamp(flow_up[:, :1] * self.regress_scale + self.max_val)
            )
...
if __name__ == "__main__":
    ...
    cfg = Config.fromfile(
        "/cpfs01/shared/public/users/mu.hu/monodepth/mono/configs/RAFTDecoder/vit.raft.full2t.py"
    )
