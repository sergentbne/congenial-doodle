...
class Compose(object):
    ...
    def __call__(
        ...
        for t in self.transforms:
            images, labels, intrinsics, cam_models, other_labels, transform_paras = t(
                images, labels, intrinsics, cam_models, other_labels, transform_paras
            )
    ...
    def __call__(
        ...
        if transform_paras is not None:
            transform_paras.update(
                label_scale_factor=label_scale_ratio,
                focal_scale_factor=label_scale_ratio,
            )
...
class ResizeKeepRatio(object):
    ...
    def __call__(
        ...
        if transform_paras is not None:
            pad_old = (
                transform_paras["pad"] if "pad" in transform_paras else [0, 0, 0, 0]
            )
            new_pad = [
                pad_old[0] + pad[0],
                pad_old[1] + pad[1],
