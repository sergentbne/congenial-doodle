...
def creat_uv_mesh(H, W):
    y, x = np.meshgrid(
        np.arange(0, H, dtype=np.float), np.arange(0, W, dtype=np.float), indexing="ij"
    )
...
class DIODEDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(DIODEDataset, self).__init__(cfg=cfg, phase=phase, **kwargs)
