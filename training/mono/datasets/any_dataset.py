...
class AnyDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(AnyDataset, self).__init__(cfg=cfg, phase=phase, **kwargs)
        ...
        self.data_root = os.path.join(
            self.mldb_info["mldb_root"], self.mldb_info["data_root"]
        )
        # depth/disp data root
        disp_root = (
            self.mldb_info["disp_root"] if "disp_root" in self.mldb_info else None
        )
        self.disp_root = (
            os.path.join(self.mldb_info["mldb_root"], disp_root)
            if disp_root is not None
            else None
        )
        depth_root = (
            self.mldb_info["depth_root"] if "depth_root" in self.mldb_info else None
        )
        self.depth_root = (
            os.path.join(self.mldb_info["mldb_root"], depth_root)
            if depth_root is not None
            else self.data_root
        )
        # meta data root
        meta_data_root = (
            self.mldb_info["meta_data_root"]
            if "meta_data_root" in self.mldb_info
            else None
        )
        self.meta_data_root = (
            os.path.join(self.mldb_info["mldb_root"], meta_data_root)
            if meta_data_root is not None
            else None
        )
        # semantic segmentation labels root
        sem_root = (
            self.mldb_info["semantic_root"]
            if "semantic_root" in self.mldb_info
            else None
        )
        self.sem_root = (
            os.path.join(self.mldb_info["mldb_root"], sem_root)
        ...
        sample_list_of_whole_data = random.sample(
            list(range(whole_data_size)), whole_data_size
        )
        self.annotations = {
            "files": [annotations["files"][i] for i in sample_list_of_whole_data]
        }
    ...
    def get_data_for_test(self, idx: int):
        ...
        curr_rgb_path = os.path.join(
            self.data_root, anno["CAM_FRONT_RIGHT"]["rgb"]
        )  # Lyft: CAM_FRONT_LEFT
        curr_depth_path = os.path.join(
            self.depth_root, anno["CAM_FRONT_RIGHT"]["depth"]
        )
        ...
        curr_cam_model = self.create_cam_model(
            curr_rgb.shape[0], curr_rgb.shape[1], ori_curr_intrinsic
        )
        # load tmpl rgb info
        ...
        rgbs, depths, intrinsics, cam_models, other_labels, transform_paras = (
            self.img_transforms(
                images=[
                    curr_rgb,
                ],
...
if __name__ == "__main__":
    ...
    cfg = Config.fromfile(
        "mono/configs/Apolloscape_DDAD/convnext_base.cascade.1m.sgd.mae.py"
    )
