...
class ScanNetDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(ScanNetDataset, self).__init__(cfg=cfg, phase=phase, **kwargs)
    ...
    def load_batch(self, meta_data, data_path, test_mode):
        ...
        curr_rgb, curr_depth = self.load_rgb_depth(
            data_path["rgb_path"], data_path["depth_path"], test_mode
        )
        # get semantic labels
    ...
    def load_norm_label(self, norm_path, H, W, test_mode):
        if norm_path is None:
            ...
            valid_mask = np.logical_not(
                np.logical_and(
                    np.logical_and(mask_gt[:, :, 0] == 0, mask_gt[:, :, 1] == 0),
                    mask_gt[:, :, 2] == 0,
                )
            )
            ...
            norm_valid_mask = (
                np.linalg.norm(norm_gt, axis=2, keepdims=True) > 0.5
            ) * valid_mask
