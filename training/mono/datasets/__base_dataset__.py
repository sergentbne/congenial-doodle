...
class BaseDataset(Dataset):
    def __init__(self, cfg, phase, **kwargs):
        ...
        self.data_root = os.path.join(
            self.db_info["db_root"], self.db_info["data_root"]
        )
        # depth/disp data root
        disp_root = self.db_info["disp_root"] if "disp_root" in self.db_info else None
        self.disp_root = (
            os.path.join(self.db_info["db_root"], disp_root)
            if disp_root is not None
            else None
        )
        depth_root = (
            self.db_info["depth_root"] if "depth_root" in self.db_info else None
        )
        self.depth_root = (
            os.path.join(self.db_info["db_root"], depth_root)
            if depth_root is not None
            else self.data_root
        )
        # meta data root
        meta_data_root = (
            self.db_info["meta_data_root"] if "meta_data_root" in self.db_info else None
        )
        self.meta_data_root = (
            os.path.join(self.db_info["db_root"], meta_data_root)
            if meta_data_root is not None
            else None
        )
        # semantic segmentation labels root
        sem_root = (
            self.db_info["semantic_root"] if "semantic_root" in self.db_info else None
        )
        self.sem_root = (
            os.path.join(self.db_info["db_root"], sem_root)
            if sem_root is not None
            else None
        )
        # depth valid mask labels root
        depth_mask_root = (
            self.db_info["depth_mask_root"]
            if "depth_mask_root" in self.db_info
            else None
        )
        self.depth_mask_root = (
            os.path.join(self.db_info["db_root"], depth_mask_root)
            if depth_mask_root is not None
            else None
        )
        # surface normal labels root
        norm_root = (
            self.db_info["normal_root"] if "normal_root" in self.db_info else None
        )
        self.norm_root = (
            os.path.join(self.db_info["db_root"], norm_root)
            if norm_root is not None
            else None
        )
        # data annotations path
        self.data_annos_path = os.path.join(
            self.db_info["db_root"], self.db_info["%s_annotations_path" % phase]
        )
        ...
        self.sample_size = (
            int(whole_data_size * cfg_sample_ratio)
            if cfg_sample_size == -1
            else (
                cfg_sample_size
                if cfg_sample_size < whole_data_size
                else whole_data_size
            )
        )
        random.seed(100)  # set the random seed
        sample_list_of_whole_data = random.sample(
            list(range(whole_data_size)), self.sample_size
        )

        self.data_size = self.sample_size
        self.annotations = {
            "files": [self.data_info["files"][i] for i in sample_list_of_whole_data]
        }
        ...
        self.depth_range = kwargs[
            "depth_range"
        ]  # predefined depth range for the network
        self.clip_depth_range = kwargs[
            "clip_depth_range"
        ]  # predefined depth range for data processing
        ...
        self.data_type = (
            cfg.data_type
        )  # there are mainly four types, i.e. ['rel', 'sfm', 'stereo', 'lidar']
        self.logger = logging.getLogger()
        self.logger.info(
            f"{self.data_name} in {self.phase} whole data size: {whole_data_size}"
        )
    ...
    def get_data_for_trainval(self, idx: int):
        ...
        curr_rgb, curr_depth, curr_normal, curr_sem, curr_cam_model = (
            data_batch["curr_rgb"],
            data_batch["curr_depth"],
            data_batch["curr_normal"],
            data_batch["curr_sem"],
            data_batch["curr_cam_model"],
        )
        ...
        if "curr_stereo_depth" in data_batch.keys():
            ...
            curr_stereo_depth = self.load_stereo_depth_label(
                None, H=curr_rgb.shape[0], W=curr_rgb.shape[1]
            )
        ...
        rgbs, depths, intrinsics, cam_models, normals, other_labels, transform_paras = (
            self.img_transforms(
                images=[
                    curr_rgb,
                ],
        ...
        stereo_depth_pre_trans = (
            other_labels[1] * (other_labels[1] > 0.3) * (other_labels[1] < 200)
        )
    ...
    def get_data_for_test(self, idx: int):
        ...
        data_batch = self.load_batch(meta_data, data_path)
        # load data
        curr_rgb, curr_depth, curr_normal, curr_cam_model = (
            data_batch["curr_rgb"],
            data_batch["curr_depth"],
            data_batch["curr_normal"],
            data_batch["curr_cam_model"],
        ...
        rgbs, depths, intrinsics, cam_models, _, other_labels, transform_paras = (
            self.img_transforms(
                images=[
                    curr_rgb,
                ],  # + tmpl_rgbs,
        ...
        depth_out = (
            self.clip_depth(curr_depth) * self.depth_range[1]
        )  # self.clip_depth(depths[0]) #
        inv_depth = self.depth2invdepth(
            depth_out, np.zeros_like(depth_out, dtype=np.bool)
        )
        filename = os.path.basename(meta_data["rgb"])[:-4] + ".jpg"
        curr_intrinsic_mat = self.intrinsics_list2mat(intrinsics[0])
        ...
        pad = transform_paras["pad"] if "pad" in transform_paras else [0, 0, 0, 0]
        scale_ratio = (
            transform_paras["label_scale_factor"]
    ...
    def load_data_path(self, meta_data):
        ...
        curr_sem_path = (
            ...
            if self.sem_root is not None
            and ("sem" in meta_data)
            and (meta_data["sem"] is not None)
            else None
        )
        # matterport3d separates xyz into three images
        if (
            ("normal" in meta_data)
            and (meta_data["normal"] is not None)
            and (self.norm_root is not None)
        ):
        ...
        if (
            ("disp" in meta_data)
            and (meta_data["disp"] is not None)
            and (self.disp_root is not None)
        ):
    ...
    def load_batch(self, meta_data, data_path):
        curr_intrinsic = meta_data["cam_in"]
        # load rgb/depth
        curr_rgb, curr_depth = self.load_rgb_depth(
            data_path["rgb_path"], data_path["depth_path"]
        )
        # get semantic labels
        curr_sem = self.load_sem_label(data_path["sem_path"], curr_depth)
        # create camera model
        curr_cam_model = self.create_cam_model(
            curr_rgb.shape[0], curr_rgb.shape[1], curr_intrinsic
        )
        # get normal labels
        curr_normal = self.load_norm_label(
            data_path["normal_path"], H=curr_rgb.shape[0], W=curr_rgb.shape[1]
        )
        # get depth mask
        ...
        # get stereo depth
        curr_stereo_depth = self.load_stereo_depth_label(
            data_path["disp_path"], H=curr_rgb.shape[0], W=curr_rgb.shape[1]
        )
    ...
    def clip_depth(self, depth: np.array) -> np.array:
        depth[
            (depth > self.clip_depth_range[1]) | (depth < self.clip_depth_range[0])
        ] = -1
