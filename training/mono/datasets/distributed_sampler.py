...
class CustomerMultiDataSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        self.logger.info(
            "Sample list of {} in rank {} is: {}".format(
                self.phase,
                self.global_rank,
                " ".join(map(str, self.sample_indices_array[-20:-10])),
            )
        )
        # subsample, each rank sample a subset for training.
        rank_offset = self.each_gpu_size * self.global_rank
        rank_indices = self.sample_indices_array[
            rank_offset : rank_offset + self.each_gpu_size
        ]
    ...
    def create_samplers(self):
        ...
        for gi, dataset_group in enumerate(self.dataset_merge.datasets):
            ...
            expand_indices_sizes = (np.array(group_cumulative_sizes) * cp_times).astype(
                np.int
            )
            ...
            for di, dataset_i in enumerate(dataset_group.datasets):
                ...
                dataset_i_ori_sample_list = (
                    self.dataset_merge.datasets[gi].datasets[di].sample_list
                )
                if self.phase == "train":
                ...
                indices_expand_list += random.sample(sample_list_i, len(dataset_i))[
                    : expand_size_i % len(dataset_i)
                ]
                # adjust indices value
                indices_expand_list = (
                    np.array(indices_expand_list)
                    + dataset_cumulative_sizes[gi]
                    + group_cumulative_sizes[di]
                )
                indices_list = (
                    np.array(sample_list_i)
                    + dataset_cumulative_sizes[gi]
                    + group_cumulative_sizes[di]
                )
                ...
                if main_process():
                    self.logger.info(
                        f'"{dataset_i.data_name}", {self.phase} set in group {gi}: '
        ...
        if "train" in self.phase:
            ...
            self.sample_indices_array = (
                np.array(self.indices_expand_mat).transpose(1, 0).reshape(-1)
            )
        ...
        self.each_gpu_size = int(
            np.ceil(self.total_indices_size * 1.0 / self.world_size)
        )  # ignore some residual samples
        self.total_dist_size = self.each_gpu_size * self.world_size
        # add extra samples to make it evenly divisible
        diff_size = int(
            self.total_dist_size - self.total_indices_size
        )  # int(self.total_dist_size - self.total_sample_size)
        if diff_size > 0:
            self.sample_indices_array = np.append(
                self.sample_indices_array, self.sample_indices_array[:diff_size]
            )
        # if main_process():
        self.logger.info(
...
def build_data_array(cfg, phase):
    ...
    for group_i in dataname_array:
        ...
        for data_i in group_i:
            ...
            dataset_i = obj_cls(
                args[data_i_cfg_name], phase, db_info=data_i_db_info, **cfg.data_basic
            )
    ...
    if main_process():
        ...
        logger.info(
            f"{phase}: data array ({data_array_names_for_log}) has been constructed."
        )
...
def log_canonical_transfer_info(cfg):
    ...
    logger.info(
        ">>>>>>>>>>>>>>Some data transfer details during augmentation.>>>>>>>>>>>>>>"
    )
    for data_i in data:
        data_i_cfg = cfg[data_i]
        ...
        for ori_f in ori_focal:
            ...
            img_size = (
                data_i_cfg.original_size[0] * scalor,
                data_i_cfg.original_size[1] * scalor,
            )
            log_str += (
                "To canonical space: focal length, %f -> %f; size, %s -> %s\n"
