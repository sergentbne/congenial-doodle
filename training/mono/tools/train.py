...
def parse_args():
    ...
    parser.add_argument(
        "--experiment_name", default="debug", help="the experiment name for mlflow"
    )
...
def main(args):
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join(
            "./work_dirs", osp.splitext(osp.basename(args.config))[0], args.timestamp
        )
...
def main_worker(local_rank: int, cfg: dict, launcher: str = "slurm"):
    ...
    if cfg.distributed:
        if launcher == "slurm":
            torch.set_num_threads(
                8
            )  # without it, the spawn method is much slower than the launch method
            cfg.dist_params.global_rank = (
                cfg.dist_params.node_rank * cfg.dist_params.num_gpus_per_node
                + local_rank
            )
    ...
    if cfg.seed is not None:
        logger.info(
            f"Set random seed to {cfg.seed}, deterministic: {cfg.deterministic}"
        )
