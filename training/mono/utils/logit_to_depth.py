...
class SoftWeight(nn.Module):
    ...
    def __init__(self, depth_bins_border):
        ...
        self.register_buffer(
            "depth_bins_border", torch.tensor(depth_bins_border), persistent=False
        )
    ...
    def forward(self, pred_logit):
        ...
        depth_log = torch.sum(
            pred_score_weight, dim=3, dtype=torch.float32, keepdim=True
        )
        depth = 10**depth_log
...
def soft_weight(pred_logit, depth_bins_border):
    ...
    if type(depth_bins_border).__module__ != torch.__name__:
        depth_bins_border = torch.tensor(
            depth_bins_border, dtype=torch.float32, device="cuda"
        )

    pred_score = nn.functional.softmax(pred_logit, dim=1)
    depth_bins_ch = pred_score.permute(
        0, 2, 3, 1
    )  # [b, h, w, c]    depth = torch.sum(depth, dim=3, dtype=torch.float32, keepdim=True)
    depth = 10**depth
    depth = depth.permute(0, 3, 1, 2)  # [b, 1, h, w]
...
if __name__ == "__main__":
    ...
    depth_bins_border = [
        np.log10(depth_min) + depth_bin_interval * (i + 0.5) for i in range(200)
    ]
