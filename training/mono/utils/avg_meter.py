...
class MetricAverageMeter(AverageMeter):
    ...
    def update_metrics_gpu(
        ...
        if is_distributed:
            (
                dist.all_reduce(delta1_sum),
                dist.all_reduce(delta2_sum),
                dist.all_reduce(delta3_sum),
            )
        ...
        consistency_rel_sum, valid_warps = get_video_consistency_err(
            pred, pred_next, pose_f1_to_f2, intrinsic
        )
        if is_distributed:
...
def get_absrel_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def get_sqrel_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def get_log10_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def get_rmse_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def get_rmse_log_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def get_silog_err(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
    ...
    diff_log_square_sum = torch.sum(
        diff_log_square.reshape((b, c, -1)), dim=2
    )  # [b, c]
    num = torch.sum(mask.reshape((b, c, -1)), dim=2)  # [b, c]
    silog = torch.sqrt(
...
def get_ratio_error(pred: torch.tensor, target: torch.tensor, mask: torch.tensor):
    """
...
def unproj_pcd(depth: torch.tensor, intrinsic: torch.tensor):
...
def forward_warp(
    ...
    for i in range(B):
        proj_depth[
            i, :, y_img_coor[i, ...][mask[i, ...]], x_img_coor[i, ...][mask[i, ...]]
        ] = Z[i, ...][mask[i, ...]]
...
def get_video_consistency_err(
    ...
    cam_coords = pixel2cam(
        pred_f1.squeeze(1).float(), intrinsic.inverse().float()
    )  # [B,3,H,W]
    ...
    f2_pixel_coords, warped_depth_f1_to_f2 = cam2pixel2(
        cam_coords, rot, tr, padding_mode="zeros"
    )  # [B,H,W,2]

    projected_depth = F.grid_sample(
        pred_f2, f2_pixel_coords, padding_mode="zeros", align_corners=False
    )
    ...
    consistency_rel_err, valid_pix = get_absrel_err(
        warped_depth_f1_to_f2, projected_depth, mask_valid
    )
...
if __name__ == "__main__":
    ...
    gt_depth = (
        np.random.random([2, 480, 640]) - 0.5
    )  # np.ones_like(pred_depth) * (-1) #
