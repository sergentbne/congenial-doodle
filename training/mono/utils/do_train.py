...
from mono.utils.running import (
    build_lr_schedule_with_cfg,
    build_optimizer_with_cfg,
    load_ckpt,
    save_ckpt,
)
...
from mono.datasets.distributed_sampler import (
    build_dataset_n_sampler_with_cfg,
    build_data_array,
)
...
def do_train(local_rank: int, cfg: dict):
    ...
    if isinstance(val_dataset, list):
        val_dataloader = [
            ...
            for val_group in val_dataset
            for val_dataset in val_group
        ]
    else:
    ...
    if cfg.load_from and cfg.resume_from is None:
        model, _, _, loss_scaler = load_ckpt(
            cfg.load_from,
            model,
            optimizer=None,
            scheduler=None,
...
def train_by_iters(
    cfg, model, optimizer, lr_scheduler, train_dataloader, val_dataloader
):
    """
    ...
    if main_process():
        training_stats = TrainingStats(
            log_period=cfg.log_interval, tensorboard_logger=tb_logger
        )
            ...
            if (
                ...
                if isinstance(val_dataloader, list):
                    val_err = validate_multiple_dataset(
                        cfg, step + 1, model, val_dataloader, tb_logger
                    )
                else:
...
def train_by_iters_amp(
    cfg, model, optimizer, lr_scheduler, train_dataloader, val_dataloader, loss_scaler
):
    """
    Do the training by iterations.
    ...
    try:
        ...
        while True:
            ...
            if not math.isfinite(total_loss):
                logger.info(
                    "Loss is {}, skiping this batch training".format(total_loss)
                )
                continue
                ...
                try:
                    if (step + 1 - start_iter) % acc_batch == 0:
                        torch.nn.utils.clip_grad_norm_(
                            model.parameters(), 2.5, error_if_nonfinite=True
                        )
                        optimizer.step()
                except:
            ...
            if (step + 1 - start_iter) % acc_batch == 0:
                ...
                if main_process():
                    training_stats.update_iter_stats(loss_dict_reduced)
                    training_stats.IterToc()
                    training_stats.log_iter_stats(
                        step // acc_batch, optimizer, max_iters, val_err
                    )
...
def set_random_crop_size_for_iter(
    dataloader: torch.utils.data.dataloader.DataLoader, iter: int, size_pool=None
):
