...
def get_prediction(
    ...
    if flip_aug == True:
        output_flip = model.module.inference(
            dict(
        ...
        if clip_range != None:
            output["prediction"] = torch.clamp(
                output["prediction"], clip_range[0], clip_range[1]
            )
            output_flip["prediction"] = torch.clamp(
                output_flip["prediction"],
                clip_range[0] / normalize_scale * scale_info,
                clip_range[1] / normalize_scale * scale_info,
            )

        output["prediction"] = 0.5 * (
            output["prediction"] + torch.flip(output_flip["prediction"], [3])
        )
        output["confidence"] = 0.5 * (
            output["confidence"] + torch.flip(output_flip["confidence"], [3])
        )
    ...
    pred_depth = torch.nn.functional.interpolate(
        pred_depth[None, None, :, :], gt_depth.shape, mode="bilinear"
    ).squeeze()  # to orginal size
    pred_depth = pred_depth * normalize_scale / scale_info

    if clip_range != None:
        pred_depth = torch.clamp(pred_depth, clip_range[0], clip_range[1])

    pred_depth_scale, scale = align_scale(
        pred_depth, gt_depth
    )  # align_scale_shift(pred_depth, gt_depth)
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        if i % save_interval == 0:
            ...
            rgb = data["input"][
                :,
                :,
                pad_info[0] : data["input"].shape[2] - pad_info[1],
                pad_info[2] : data["input"].shape[3] - pad_info[3],
            ]
            rgb = torch.nn.functional.interpolate(
                rgb, gt_depth.shape, mode="bilinear"
            ).squeeze()
            max_scale = save_val_imgs(
                i,
                pred_depth,
                gt_depth,
                rgb,
        ...
        if "normal_out_list" in output.keys():
            ...
            pred_normal = pred_normal[
                :, :, pad_info[0] : H - pad_info[1], pad_info[2] : W - pad_info[3]
            ]
