...
class ToTensor(object):
    ...
    def __call__(
        ...
        if (
            ...
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle inputs/labels/intrinsics lists."
                )
            )
        if len(images) != len(intrinsics):
            raise (RuntimeError("Numbers of images and intrinsics are not matched."))
        if not isinstance(images[0], np.ndarray) or not isinstance(
            labels[0], np.ndarray
        ):
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle np.ndarray for the input and label."
        ...
        if not isinstance(intrinsics[0], list):
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle list for the camera intrinsics"
                )
            )
        ...
        if len(images[0].shape) > 3 or len(images[0].shape) < 2:
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle image(np.ndarray) with 3 dims or 2 dims.\n"
                )
            )
        if len(labels[0].shape) > 3 or len(labels[0].shape) < 2:
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle label(np.ndarray) with 3 dims or 2 dims.\n"
                )
            )
        ...
        if len(intrinsics[0]) > 4 or len(intrinsics[0]) < 3:
            raise (
                RuntimeError(
                    "transform.ToTensor() only handle intrinsic(list) with 3 sizes or 4 sizes.\n"
                )
            )
        ...
        if cam_models is not None:
            for i, cam_model in enumerate(cam_models):
                cam_models[i] = (
                    torch.from_numpy(cam_model.transpose((2, 0, 1))).float()
...
class Normalize(object):
    ...
    def __init__(self, mean, std=None, **kwargs):
        ...
        self.std = (
            torch.tensor(std).float()[:, None, None]
            if std is not None
...
class ResizeCanonical(object):
    ...
    def random_on_canonical_transform(
        self, image, label, intrinsic, cam_model, to_random_ratio
    ):
        ...
        if intrinsic is not None:
            intrinsic = [
                self.canonical_focal,
                self.canonical_focal,
                intrinsic[2] * resize_ratio,
                intrinsic[3] * resize_ratio,
            ]
        if label is not None:
            # number of other labels may be less than that of image
            label = cv2.resize(
                label, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_NEAREST
            )
    ...
    def random_on_crop_transform(
        ...
        resize_ratio = min(
            resize_ratio_h, resize_ratio_w
        )  # resize based on the long edge
        reshape_h = int(ori_h * resize_ratio + 0.5)
        reshape_w = int(ori_w * resize_ratio + 0.5)
    ...
    def __call__(
        ...
        for i in range(len(images)):
            ...
            cam_model = (
                cam_models[i]
                if cam_models is not None and i < len(cam_models)
                else None
            )
            img, label, intrinsic, cam_model, to_scale_ratio = (
                self.random_on_canonical_transform(
        ...
        if normals != None:
            ...
            for i, normal in enumerate(normals):
                normals[i] = cv2.resize(
                    normal,
                    dsize=(reshape_w, reshape_h),
...
class LabelScaleCononical(object):
    ...
    def __call__(
        ...
        for i in range(len(intrinsics)):
            ...
            intrinsics[i] = [
                intrinsic_i[0] * label_scale_ratio,
                intrinsic_i[1] * label_scale_ratio,
                intrinsic_i[2],
                intrinsic_i[3],
            ]
...
class ResizeKeepRatio(object):
    ...
    def __init__(self, resize_size, padding=None, ignore_label=-1, **kwargs):
        if isinstance(resize_size, int):
            ...
            isinstance(resize_size, collections.Iterable)
            and len(resize_size) == 2
            and isinstance(resize_size[0], int)
            and isinstance(resize_size[1], int)
            and resize_size[0] > 0
            and resize_size[1] > 0
        ):
    ...
    def __call__(
        ...
        to_scale_ratio = self.get_label_scale_factor(
            images[0], intrinsics[0], resize_ratio
        )
        ...
        if transform_paras is not None:
            ...
            if "label_scale_factor" in transform_paras:
                transform_paras["label_scale_factor"] = (
                    transform_paras["label_scale_factor"] * 1.0 / to_scale_ratio
                )
            else:
                transform_paras.update(label_scale_factor=1.0 / to_scale_ratio)
        return (
...
class RandomCrop(object):
    ...
    def __init__(
        self, crop_size, crop_type="center", padding=None, ignore_label=-1, **kwargs
    ):
    ...
    def __call__(
        ...
        if (
            "random_crop_size" in transform_paras
            and transform_paras["random_crop_size"] is not None
            and (
                transform_paras["random_crop_size"][0]
                + transform_paras["random_crop_size"][1]
                > 500
            )
        ):
        ...
        pad_h, pad_w, pad_h_half, pad_w_half = self.cal_padding_paras(
            target_h, target_w
        )
        h_off, w_off = self.cal_cropping_paras(
            target_h + pad_h, target_w + pad_w, target_intrinsic
        )
    ...
    def __call__(
        ...
        if normals != None:
            ...
            for i, norm in enumerate(normals):
                normals[i] = cv2.resize(
                    norm, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_NEAREST
                )
        ...
        if transform_paras is not None:
            if "label_scale_factor" in transform_paras:
                transform_paras["label_scale_factor"] = (
                    transform_paras["label_scale_factor"] * label_scale_ratio
                )
            else:
                transform_paras.update(label_scale_factor=label_scale_ratio)
        return (
...
class RandomEdgeMask(object):
    ...
    def __call__(
        ...
        mask_edgesize = random.sample(
            range(self.mask_maxsize), 4
        )  # [up, down, left, right]
        for i in range(len(images)):
            img = images[i]
            label = labels[i] if i < len(labels) else None
        ...
        if other_labels != None:
            # other labels are like semantic segmentations, instance segmentations, instance planes segmentations...
            for i, other_label_i in enumerate(other_labels):
                other_labels[i] = self.mask_edge(
                    other_label_i, mask_edgesize, self.label_invalid
                )

        if transform_paras is not None:
            pad = transform_paras["pad"] if "pad" in transform_paras else [0, 0, 0, 0]
            new_pad = [
                max(mask_edgesize[0], pad[0]),
                max(mask_edgesize[1], pad[1]),
                max(mask_edgesize[2], pad[2]),
                max(mask_edgesize[3], pad[3]),
            ]
    ...
    def __call__(
        ...
        for i in range(len(images)):
            ...
            if transform_paras is not None:
                transform_paras.update(dict(pad=pad))
        if normals is not None:
            pad_h, pad_w, pad_h_half, pad_w_half = self.get_pad_paras(
...
class Weather(object):
    ...
    def __init__(self, prob=0.3, **kwargs):
        snow = iaa.FastSnowyLandscape(
            lightness_threshold=[50, 100], lightness_multiplier=(1.2, 2)
        )
        cloud = iaa.Clouds()
        fog = iaa.Fog()
...
if __name__ == "__main__":
    img = cv2.imread(
        "/mnt/mldb/raw/62b3ed3455e805efcb28c74b/NuScenes/data_test/samples/CAM_FRONT/n008-2018-08-01-15-34-25-0400__CAM_FRONT__1533152214512404.jpg",
        -1,
    )
    H, W, _ = img.shape
    label = img[:, :, 0]
    ...
    for i in range(20):
        ...
        img_aug, label, intrinsic, cam_model, ref_images, transform_paras = weather_aug(
            [
                img,
            ],
            [
