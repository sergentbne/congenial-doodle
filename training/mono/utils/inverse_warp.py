...
def inverse_warp2(img, depth, ref_depth, pose, intrinsics, padding_mode="zeros"):
    ...
    src_pixel_coords, computed_depth = cam2pixel2(
        cam_coords, rot, tr, padding_mode
    )  # [B,H,W,2]
...
def inverse_rotation_warp(img, rot, intrinsics, padding_mode="zeros"):
    ...
    cam_coords = pixel2cam(
        torch.ones(b, h, w).type_as(img), intrinsics.inverse()
    )  # [B,3,H,W]
    ...
    src_pixel_coords, computed_depth = cam2pixel2(
        cam_coords, proj_cam_to_src_pixel, None, padding_mode
    )  # [B,H,W,2]
    projected_img = F.grid_sample(
...
def grid_to_flow(grid):
    b, h, w, _ = grid.size()
    i_range = (
        torch.arange(0, h).view(1, h, 1).expand(1, h, w).type_as(grid)
    )  # [1, H, W]
    j_range = (
        torch.arange(0, w).view(1, 1, w).expand(1, h, w).type_as(grid)
    )  # [1, H, W]
