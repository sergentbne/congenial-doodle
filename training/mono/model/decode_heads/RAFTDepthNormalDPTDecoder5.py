...
class FlowHead(nn.Module):
    def __init__(
        self, input_dim=128, hidden_dim=256, output_dim_depth=2, output_dim_norm=4
    ):
...
class FuseBlock(nn.Module):
    # reimplementation of DPT
    def __init__(
        self, in_channels, out_channels, fuse=True, upsample=True, scale_factor=2
    ):
...
class Readout(nn.Module):
    ...
    def __init__(self, in_features, use_cls_token=True, num_register_tokens=0):
        ...
        if self.use_cls_token == True:
            ...
            self.project_learn = nn.Linear(
                (1 + num_register_tokens) * in_features, in_features, bias=False
            )
            self.act = nn.GELU()
        else:
...
class DecoderFeature(nn.Module):
    def __init__(
        self, vit_channel, num_ch_dec=[128, 256, 512, 1024, 1024], use_cls_token=True
    ):
        ...
        self.upconv_3 = FuseBlock(
            self.num_ch_dec[4], self.num_ch_dec[3], fuse=False, upsample=False
        )
        ...
        self.upconv_1 = FuseBlock(
            self.num_ch_dec[2], self.num_ch_dec[1] + 2, scale_factor=7 / 4
        )
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.context_feature_encoder = ContextFeatureEncoder(
            self.feature_channels, [self.hidden_dims, self.context_dims]
        )
        ...
        self.update_block = BasicMultiUpdateBlock(
            cfg, hidden_dims=self.hidden_dims, out_dims=6
        )
...
if __name__ == "__main__":
    ...
    cfg = Config.fromfile(
        "/mu.hu/monodepth/mono/configs/RAFTDecoder/vit.raft.full2t.py"
    )
