...
class HDSNLoss(nn.Module):
    ...
    def __init__(
        self, loss_weight=1.0, grid=3, data_type=["sfm", "stereo", "lidar"], **kwargs
    ):
            ...
            for h in range(int(1 / anchor)):
                for w in range(int(1 / anchor)):
                    ...
                    mask_new[
                        :,
                        :,
                        int(h * anchor * height) : int((h + 1) * anchor * height),
                        int(w * anchor * width) : int((w + 1) * anchor * width),
                    ] = True
    ...
    def ssi_mae(self, prediction, target, mask_valid):
        ...
        loss = torch.sum(torch.abs(gt_trans - pred_trans) * mask_valid) / (
    ...
    def forward(self, prediction, target, mask=None, **kwargs):
        ...
        prediction_hie = (
            ...
            .repeat(hierachy_masks.shape[0], 1, 1, 1, 1)
            .reshape(-1, C, H, W)
        )
