...
class EdgeguidedNormalLoss(nn.Module):
    ...
    def get_loss(self, prediction, target, mask, input, intrinsic, **kwargs):
        ...
        if "predictions_normals" not in kwargs:
            ...
            targets_normals, targets_normals_masks = self.depth2normal(
                target, intrinsic, mask
            )
        else:
        ...
        for i in range(n):
            ...
            losses += torch.sum(
                torch.abs(torch.ones_like(target_cos) - input_cos)
        ...
        if torch.isnan(loss).item() | torch.isinf(loss).item():
            ...
            print(
                f"Pair-wise Normal Regression Loss NAN error, {loss}, valid pix: {valid_samples}"
            )
        return loss
...
def tmp_check_normal(normals, masks, depth):
    ...
    vis_normal1 = vis_surface_normal(
        normals[0, ...].permute(1, 2, 0).detach(), masks[0, ...].detach().squeeze()
    )
    vis_normal2 = vis_surface_normal(
        normals[1, ...].permute(1, 2, 0).detach(), masks[1, ...].detach().squeeze()
    )
...
if __name__ == "__main__":
    ...
    loss = ENL(
        prediction, depth_t, masks=depth_t > 0, images=imgs, intrinsic=intrinsics
    )
    print(loss)
