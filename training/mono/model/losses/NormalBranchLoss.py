...
class NormalBranchLoss(nn.Module):
    def __init__(
        self,
        loss_weight=1.0,
        data_type=["sfm", "stereo", "denselidar", "denselidar_syn"],
        d2n_dataset=["ScanNetAll"],
        loss_fn="UG_NLL_ours",
        **kwargs,
    ):
    ...
    def forward_GRU(
        ...
        **kwargs,
        ...
        for i, norm_out in enumerate(normal_out_list):
            ...
            if auxi_normal != None:
                auxi_loss = self.forward_R(
                    norm_out.clone(), auxi_normal[:, :3], auxi_normal_mask
                )
    ...
    def forward_UG(self, normal_pred_list, normal_coord_list, normal, mask, **kwargs):
        ...
        for pred, coord in zip(normal_pred_list, normal_coord_list):
            if coord is None:
                pred = F.interpolate(
                    pred,
                    size=[normal.size(2), normal.size(3)],
                    mode="bilinear",
                    align_corners=True,
                )
                ...
                gt_norm_ = F.grid_sample(
                    normal, coord, mode="nearest", align_corners=True
                )  # (B, 3, 1, N)
                gt_norm_mask_ = F.grid_sample(
                    gt_normal_mask.float(), coord, mode="nearest", align_corners=True
                )  # (B, 1, 1, N)
...
class DeNoConsistencyLoss(nn.Module):
    def __init__(
        ...
        sky_id=142,
        scale=1,
        norm_dataset=["Taskonomy", "Matterport3D", "Replica", "Hypersim", "NYU"],
        no_sky_dataset=["BigData", "DIODE", "Completion", "Matterport3D"],
        disable_dataset=[],
        depth_detach=False,
        **kwargs,
    ):
    ...
    def forward(self, **kwargs):
        ...
        self.batch_enabled = torch.tensor(
            [
        ...
        self.batch_with_norm_sky = torch.tensor(
            [
                1 if batch_dataset not in self.no_sky_dataset else 0
                for batch_dataset in batches_dataset
            ],
            device=device,
            dtype=torch.bool,
    ...
    def forward_J(
        ...
        **kwargs,
        ...
        normal, new_mask = self.depth2normal(
            prediction.detach() if self.depth_detach else prediction,
            intrinsic,
            pad_mask,
        )
        # mask sky
        ...
        sample_mask_d = sample_points(
            prediction, confidence, new_mask, sampling_ratio=0.7
        )

        # all mask
        normal_mask = (
            ~torch.all(normal == 0, dim=1, keepdim=True) & new_mask & sample_mask_d
        )
    ...
    def forward_S(
        ...
        **kwargs,
        ...
        if target != None:
            ...
            # mask sky
                ...
                sky_mask = torch.nn.functional.interpolate(
                    sky_mask.float(), scale_factor=0.25
                ).bool()
                target_mask = torch.nn.functional.interpolate(
                    target_mask.float(), scale_factor=0.25
                ).bool()
                new_mask = new_mask & (
                    (sky_mask & self.batch_with_norm_sky) | target_mask
                )
        else:
        ...
        sample_mask_d = sample_points(
            prediction, confidence, new_mask, sampling_ratio=sampling_ratio
        )
        conf_mask = confidence > 0.5

        # all mask
        normal_mask = (
            ~torch.all(normal == 0, dim=1, keepdim=True)
            & new_mask
            & sample_mask_n
            & sample_mask_d
            & conf_mask
    ...
    def forward_S_GRU(
        ...
        **kwargs,
        ...
        for i, (norm, conf, depth) in enumerate(
            zip(normal_out_list, confidence_list, predictions_list)
        ):
            # We adjust the loss_gamma so it is consistent for any number of RAFT-Stereo iterations
            adjusted_loss_gamma = self.loss_gamma ** (15 / (n_predictions - 1))
            i_weight = adjusted_loss_gamma ** (n_predictions - i - 1)
            ...
            if i == 0:
                ...
                new_intrinsic = torch.cat(
                    (intrinsic[:, :2, :] / 4, intrinsic[:, 2:3, :]), dim=1
                )
                curr_loss = self.forward_S(
                    low_resolution_init[0],
                    low_resolution_init[1],
                    new_intrinsic,
                    torch.nn.functional.interpolate(
                        pad_mask.float(), scale_factor=0.25
                    ).bool(),
                    low_resolution_init[2],
                    sem_mask,
                    target,
                    is_initial_pair,
                    scale=kwargs["scale"],
    ...
    def forward_S_Search(
        ...
        **kwargs,
        ...
        prediction_normal = torch.cat(
            (
                prediction_normal[:, :2] * torch.ones_like(candidate_scale),
                prediction_normal[:, 2:3] * candidate_scale,
                prediction_normal[:, 3:4] * torch.ones_like(candidate_scale),
            ),
            dim=1,
        )
        ...
        prediction_normal[:, :3] = prediction_normal[:, :3] / (
            torch.sqrt(norm_x**2.0 + norm_y**2.0 + norm_z**2.0) + 1e-10
        )
