...
class ConfidenceGuideLoss(nn.Module):
    ...
    def __init__(
        self,
        loss_weight=1,
        data_type=["stereo", "lidar", "denselidar"],
        loss_gamma=0.9,
        conf_loss=True,
        **kwargs,
    ):
    ...
    def forward(self, samples_pred_list, target, coord_list, mask=None, **kwargs):
        ...
        for i, (pred, coord) in enumerate(zip(samples_pred_list, coord_list)):
            # coord: B, 1, N, 2
            # pred: B, 2, N
            gt_depth_ = F.grid_sample(
                target, coord, mode="nearest", align_corners=True
            )  # (B, 1, 1, N)
            gt_depth_mask_ = F.grid_sample(
                mask.float(), coord, mode="nearest", align_corners=True
            )  # (B, 1, 1, N)
            ...
            if self.conf_loss:
                ...
                gt_confidence = (
                    1 - torch.abs((pred_depth - gt_depth_) / gt_depth_)
                ) * conf_mask
                conf_loss = torch.sum(
                    torch.abs(pred_conf - gt_confidence) * conf_mask
