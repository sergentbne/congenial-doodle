...
def randomSampling(predictions, targets, masks, threshold, sample_num):
    ...
    consistent_masks_A = consistent_masks_index[
        shuffle_effect_pixels[0 : sample_num * 2 : 2]
    ]
    consistent_masks_B = consistent_masks_index[
        shuffle_effect_pixels[1 : sample_num * 2 : 2]
    ]
...
def edgeGuidedSampling(predictions, targets, edges_img, thetas_img, masks, h, w):
    ...
    index_anchors = torch.randint(
        0, minlen, (sample_num,), dtype=torch.long, device="cuda"
    )
...
class RankingLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask=None, **kwargs):
        ...
        for i in range(n):
            ...
            mask_eq = target_ratio.lt(1.0 + self.sigma) * target_ratio.gt(
                1.0 / (1.0 + self.sigma)
            )
...
class EdgeguidedRankingLoss(nn.Module):
    def __init__(
        ...
        mask_value=1e-6,
        loss_weight=1.0,
        data_type=["rel", "sfm", "stereo", "lidar"],
        **kwargs,
    ):
    ...
    def get_loss(self, prediction, target, mask=None, input=None, **kwargs):
        ...
        for i in range(n):
            ...
            inputs_A, inputs_B, targets_A, targets_B, masks_A, masks_B, sample_num = (
                edgeGuidedSampling(
                    prediction[i, :],
                    target[i, :],
                    edges_img[i],
            ...
            equal_loss = (inputs_A_merge - inputs_B_merge).pow(2)[
                mask_eq & consistency_mask
            ]
            unequal_loss = torch.log(
                1 + torch.exp((-inputs_A_merge + inputs_B_merge) * labels)
            )[(~mask_eq) & consistency_mask]
