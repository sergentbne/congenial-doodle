...
class PWNPlanesLoss(nn.Module):
    ...
    def init_image_coor(self, B, H, W):
        ...
        pixel_coords = torch.stack((u, v, ones), dim=1).expand(
            B, 3, H, W
        )  # [B, 3, H, W]
    ...
    def select_index(self, mask_kp):
        ...
        for i in range(x):
            ...
            if valid_points.shape[0] < select_size * 0.6:
                ...
                repeat_idx = torch.randperm(valid_points.shape[0], device="cuda")[
            ...
            p3_y.append(p3[:, 0])
        p123 = {
            "p1_x": torch.stack(p1_x),
            "p1_y": torch.stack(p1_y),
            "p2_x": torch.stack(p2_x),
            "p2_y": torch.stack(p2_y),
            "p3_x": torch.stack(p3_x),
            "p3_y": torch.stack(p3_y),
    ...
    def filter_mask(self, pw_pred):
        ...
        proj_query = (
            xy_diff.contiguous()
            .view(m_batchsize * groups, -1, index)
            .permute(0, 2, 1)
            .contiguous()
        )  # [bn, 3(p123), 2(xy)]
        proj_key = xy_diff.contiguous().view(
        ...
        norm_energy = norm_energy.contiguous().view(
            m_batchsize * groups, -1
        )  # [bn, 9(p123)]
        mask_cos = (
            torch.sum(
                (norm_energy > self.delta_cos) + (norm_energy < -self.delta_cos), 1
            )
            > 3
        )  # igonre
    ...
    def constrain_a_plane_loss(self, pw_groups_pre_i, mask_valid_i):
        ...
        orient_mask = (
            torch.sum(
                torch.squeeze(virtual_normal)
                * torch.squeeze(pw_groups_pred_i[:, :, 0]),
                dim=1,
            )
            > 0
        )
    ...
    def get_loss(self, pred_depth, gt_depth, ins_planes_mask, intrinsic=None):
        ...
        for i in range(B):
            ...
            planes = [
                mask_i == m for m in unique_planes if m != 0
            ]  # [x, 1, h, w] x is the planes number
            if len(planes) == 0:
                continue
            mask_planes = torch.cat(planes, dim=0)  # torch.stack(planes, dim=0) #
            pcd_grops_pred, mask_valid = self.select_points_groups(
                pcd[i, ...][None, :, :, :], mask_planes
            )  # [x, N, 3(x,y,z), 3(p1,p2,p3)]
    ...
    def forward(
        self, prediction, target, mask, intrinsic, **kwargs
...
if __name__ == "__main__":
    ...
    loss = vnl_loss(
        gt_depth, gt_depth, mask, intrins, dataset=np.array(["Taskonomy", "Taskonomy"])
    )
    print(loss)
