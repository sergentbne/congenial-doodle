...
class GRUSequenceLoss(nn.Module):
    ...
    def __init__(
        self,
        loss_weight=1,
        data_type=["lidar", "denselidar", "stereo", "denselidar_syn"],
        loss_gamma=0.9,
        silog=False,
        stereo_sup=0.001,
        stereo_dataset=["KITTI", "NYU"],
        **kwargs,
    ):
    ...
    def conf_loss(self, confidence, prediction, target, mask):
        ...
        loss = torch.sum(torch.abs(confidence - gt_confidence) * conf_mask) / (
            torch.sum(conf_mask) + self.eps
        )
        ...
        for i, prediction in enumerate(predictions_list):
            ...
            mask_stereo = 1 + torch.nn.functional.max_pool2d(
                -torch.nn.functional.max_pool2d(
                    mask * 1.0, 3, stride=1, padding=1, dilation=1
                ),
                3,
                stride=1,
                padding=1,
            ...
            stereo_depth_loss = torch.sum(
                self.batch_with_stereo * stereo_diff * mask_stereo
            ) / (torch.sum(mask_stereo) + self.eps)
