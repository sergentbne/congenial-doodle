...
class Backprojection(nn.Module):
    ...
    def __init__(self, height, width):
        ...
        self.horizontal_step = (
            self.horizontal_angle_range[1] - self.horizontal_angle_range[0]
        ) / (self.horizontal_sample_num - 1)
        self.vertical_step = (
            self.vertical_angle_range[1] - self.vertical_angle_range[0]
        ) / (self.vertical_sample_num - 1)

        self.horizontal_samples = np.arange(
            self.horizontal_angle_range[0],
            self.horizontal_angle_range[1],
            self.horizontal_step,
        )
        self.vertical_samples = np.arange(
            self.vertical_angle_range[0],
            self.vertical_angle_range[1],
            self.vertical_step,
        )
        ...
        self.dir_in_virt_cam = self.dir_in_virt_cam.reshape(
            3, self.virt_H * self.virt_W
        )
...
def get_surface_normalv2(xyz, patch_size=5, mask_valid=None):
    ...
    mask_p4p6 = (
        mask_pad[:, half_patch : half_patch + h, :w]
        & mask_pad[:, half_patch : half_patch + h, -w:]
    )
    mask_p2p8 = (
        mask_pad[:, :h, half_patch : half_patch + w]
        & mask_pad[:, -h:, half_patch : half_patch + w]
    )
    ...
    # plt.show()
    return n_img_aver_norm.permute(0, 3, 1, 2).contiguous(), mask_normal[
        :, None, :, :
    ]  # [b, h, w, 3]
