...
class PhotometricGeometricLoss(nn.Module):
    ...
    def photo_and_geometry_loss(
        self, tgt_img, ref_imgs, tgt_depth, ref_depths, intrinsics, poses, poses_inv
    ):
        diff_img_list = []
        diff_color_list = []
        diff_depth_list = []
        ...
        for ref_img, ref_depth, pose, pose_inv in zip(
            ref_imgs, ref_depths, poses, poses_inv
        ):
            (
                diff_img_tmp1,
                diff_color_tmp1,
    ...
    def compute_pairwise_loss(
        self, tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic
    ):
    ...
    def forward(
        self, input, ref_input, prediction, ref_prediction, intrinsic, **kwargs
    ):
        ...
        loss = (
            self.loss_weight_geometry * geometry_loss
            + self.loss_weight_photo * photo_loss
        )
