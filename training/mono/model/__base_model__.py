...
class BaseDepthModel(nn.Module):
    def __init__(self, cfg, criterions, **kwards):
        ...
        self.criterions_main = (
            criterions["decoder_losses"]
            if criterions and "decoder_losses" in criterions
            else None
        )
        self.criterions_auxi = (
            criterions["auxi_losses"]
            if criterions and "auxi_losses" in criterions
            else None
        )
        self.criterions_pose = (
            criterions["pose_losses"]
            if criterions and "pose_losses" in criterions
            else None
        )
        self.criterions_gru = (
            criterions["gru_losses"]
            if criterions and "gru_losses" in criterions
            else None
        )
        try:
    ...
    def compute_gru_loss(self, paras_):
        ...
        paras = {
            k: v
            for k, v in paras_.items()
            if k != "prediction" and k != "prediction_normal"
        }
    ...
    def compute_decoder_loss(self, paras):
        ...
        decode_losses_dict = self.branch_loss(
            criterions=self.criterions_main, branch="decode", **paras
        )
    ...
    def branch_loss(
        self, prediction, pred_logit, criterions, branch="decode", **kwargs
    ):
        ...
        for loss_method in criterions:
            ...
            loss_tmp = loss_method(
                prediction=prediction, target=target, mask=new_mask, **args
            )
    ...
    def create_mask_as_loss(self, loss_method, mask, batches_data_type):
        ...
        batch_mask = torch.tensor(
            np.any(data_type_req == batches_data_type, axis=0), device="cuda"
        )  # torch.from_numpy(np.any(data_type_req == batches_data_type, axis=0)).cuda()
    ...
    def label_downsample(self, downsample_factor, data_dict):
        ...
        downsample_target = F.interpolate(
            data_dict["target"], scale_factor=scale_factor
        )
        downsample_stereo_depth = F.interpolate(
            data_dict["stereo_depth"], scale_factor=scale_factor
        )
    ...
    def pred_upsample(self, downsample_factor, data_dict):
        ...
        upsample_prediction = F.interpolate(
            data_dict["prediction"], scale_factor=scale_factor
        ).detach()
        upsample_confidence = F.interpolate(
            data_dict["confidence"], scale_factor=scale_factor
        ).detach()
