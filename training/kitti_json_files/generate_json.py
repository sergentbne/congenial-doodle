...
if True:
    ...
    for l in lines:
        ...
        depth_train = os.path.join(
            depth_root, "train", scene_no_date, mid, cam, frame + ".png"
        )
        depth_val = os.path.join(
            depth_root, "val", scene_no_date, mid, cam, frame + ".png"
        )
        rgb = os.path.join(raw_root, scene, cam, mid_raw, frame + ".png")

        with open(
            os.path.join(common_root, raw_root, date, "calib_cam_to_cam.txt")
        ) as c:
            lines_c = c.readlines()
